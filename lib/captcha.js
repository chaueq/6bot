...
async function captchaSolvingKernel(e) {
    ...
    console.log('Captcha solution: ' + extractString(result));
    submitSolution(extractString(result));

    // while(captchaAppeared() && (!captchaError() || captchaProcessing())) {
    //     await sleep(100);
    // }
    ...
    //     let best = 0;
    //     for(let i=1; i < 7; ++i) {
    //         if(result[i].second.diff < result[best].second.diff) {
    //             best = i;
    //         }
    //     }

    //     result[best].char = result[best].second.char;
    //     result[best].dx = result[best].second.dx;
    //     result[best].code = result[best].second.code;
    //     startX = 0;
    //     while(counter[startX] == 0){
    //         ++startX;
    //     }
    //     for(let i = 0; i <= best; ++i) {
    //         startX += result[i].code.length + result[i].dx;
    //     }
    //     for(let i = best; i < 7; ++i) {
    //         let letter = recognizeLetter(counter, startX);
    //         startX += letter.code.length + letter.dx;
    //         result[i] = letter;
    //     }
    ...
    //     await minDelay;
    //     console.log('Secondary captcha solution: ' + extractString(result));
    //     submitSolution(extractString(result));
    // }
...
function recognizeLetter(data, start) {
    ...
    for(let dx = -ampX; dx <= ampX; ++dx) {
        ...
        else if(best_dx_score < secondDiff) {
            secondChar = best_dx_char;
            secondDiff = best_dx_score;
            secondChar.dx = best_dx;
...
function getPatterns() {
    return [
        {char: 'a', code: [5,15,15,15,42,42,42,42,42,43,43,48,48,48,48,48,48,43,43,43,42,42,42,42,42,43,43,45,48,49,51,51,48,47,45,43,42,42,42,42,42,42,39,35,34,33,37,37]},
        {char: 'b', code: []},
        ...
        {char: 'l', code: []},
        {char: 'm', code: [42,42,42,42,42,43,45,47,46,44,44,45,43,42,42,42,42,42,42,42,42,42,42,42,42,42,42,42,37,37,37,37,37,43,43,43,43,43,43,43,43,37,37,37,37,37,29,29]},
        ...
        {char: 'q', code: []},
        {char: 'r', code: [14,16,17,42,42,42,42,42,42,42,42,42,42,47,46,47,42,42,42,36,36,36,36,36,36,36,36,36,36,41,40,41,41,36,36,36,36,36,14]},
        {char: 's', code: []},
        {char: 't', code: []},
        ...
        {char: 'v', code: []},
        {char: 'w', code: [18,19,31,32,31,32,42,42,42,42,42,42,42,42,42,42,42,42,42,42,42,42,46,46,46,46,46,46,46,46,46,48,54,54,53,54,53,46,46,46,46,46,46,51,51,51,50,46,46]},
